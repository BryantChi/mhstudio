...
2. **提示條必須帶 `alert-permanent`**：`resources/js/app.js:73-80` 會把頁面上所有 `.alert` 在 5 秒後自動關閉，只有帶 `alert-permanent` 的例外。客戶詳情頁的暫定提示條含「轉為正式客戶」按鈕，少了這個 class 會在 5 秒後連按鈕一起消失——這是實機操作才發現的，`assertSee` 類的測試抓不到。日後在後台加任何「常駐狀態提示」都要記得這件事。

3. **Select2 的 `insertTag` 不可省略**：預設會把「＋ 用「xxx」開單」插在候選清單**最前面**，使用者打字後按 Enter 會誤建一筆名稱不完整的客戶（例如只打「永」就建出「永」這個客戶）。partial 內以 `insertTag: (data, tag) => data.push(tag)` 把它改到最後，確保既有客戶優先。

4. **新增與客戶相關的功能時不必特判暫定客戶**：暫定客戶就是一筆正常的 Client，差別只在 `is_provisional` 這個顯示用旗標。唯一要記得的是客戶列表的篩選預設值。

5. **暫定客戶不會自動清除**：若累積過多未使用的暫定客戶，需另行處理（可依「無任何報價／合約／發票關聯且為暫定」條件清理），目前不實作。
