...
<div class="row mb-4">
    <div class="col-12">
        <h2 class="mb-0">編輯合約範本</h2>
        <p class="text-muted">{{ $contractTemplate->name }}</p>
    </div>
</div>

<form method="POST" action="{{ route('admin.contract-templates.update', $contractTemplate) }}" onsubmit="showLoading()">
    @csrf @method('PUT')
    <div class="row">
        <div class="col-lg-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="description" name="description" rows="3">{{ old('description', $contractTemplate->description) }}</textarea>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                            @foreach(['service' => '服務合約', 'maintenance' => '維護合約', 'retainer' => '長期顧問', 'nda' => '保密協議', 'other' => '其他'] as $val => $label)
                                <option value="{{ $val }}" {{ old('type', $contractTemplate->type) == $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" class="form-control" id="default_amount" name="default_amount"
                                   value="{{ old('default_amount', $contractTemplate->default_amount) }}" step="0.01" min="0" placeholder="0.00">
                        </div>
                    </div>
                    <div class="mb-3">
                        <label for="order" class="form-label">排序</label>
                        <input type="number" class="form-control" id="order" name="order"
                               value="{{ old('order', $contractTemplate->order) }}" readonly>
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-primary">
                            <svg class="icon me-2"><use xlink:href="/assets/icons/free.svg#cil-save"></use></svg> 更新範本
                        </button>
