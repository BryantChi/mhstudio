...
<div class="row mb-4">
    <div class="col-12">
        <h2 class="mb-0">新增客戶</h2>
        <p class="text-muted">建立新的客戶檔案</p>
    </div>
</div>

<form method="POST" action="{{ route('admin.clients.store') }}" onsubmit="showLoading()" data-form-confirm>
    @csrf
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('tax_id') is-invalid @enderror"
                                   id="tax_id" name="tax_id" value="{{ old('tax_id') }}" placeholder="例如：12345678" maxlength="20">
                            @error('tax_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="contact_person" class="form-label">聯繫人</label>
                            <input type="text" class="form-control @error('contact_person') is-invalid @enderror"
                                   id="contact_person" name="contact_person" value="{{ old('contact_person') }}" placeholder="主要聯繫人姓名">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('phone') is-invalid @enderror"
                                   id="phone" name="phone" value="{{ old('phone') }}" placeholder="例如：0912-345-678">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                            <option value="lead" {{ old('status', 'lead') == 'lead' ? 'selected' : '' }}>潛在客戶</option>
                            <option value="active" {{ old('status') == 'active' ? 'selected' : '' }}>活躍</option>
                            <option value="inactive" {{ old('status') == 'inactive' ? 'selected' : '' }}>不活躍</option>
                            <option value="archived" {{ old('status') == 'archived' ? 'selected' : '' }}>已歸檔</option>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('tier') is-invalid @enderror" id="tier" name="tier" required>
                            <option value="standard" {{ old('tier', 'standard') == 'standard' ? 'selected' : '' }}>標準</option>
                            <option value="premium" {{ old('tier') == 'premium' ? 'selected' : '' }}>高級</option>
                            <option value="vip" {{ old('tier') == 'vip' ? 'selected' : '' }}>VIP</option>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('source') is-invalid @enderror" id="source" name="source" required>
                            <option value="">-- 請選擇 --</option>
                            <option value="website" {{ old('source') == 'website' ? 'selected' : '' }}>網站</option>
                            <option value="referral" {{ old('source') == 'referral' ? 'selected' : '' }}>推薦</option>
                            <option value="social" {{ old('source') == 'social' ? 'selected' : '' }}>社群媒體</option>
                            <option value="cold_outreach" {{ old('source') == 'cold_outreach' ? 'selected' : '' }}>主動開發</option>
