...
<div class="row mb-4">
    <div class="col-md-6">
        <h2 class="mb-0">編輯服務</h2>
        <p class="text-muted">修改服務項目內容</p>
    </div>
    <div class="col-md-6 text-md-end">
        <form method="POST"
              action="{{ route('admin.services.destroy', $service) }}"
              class="d-inline"
              onsubmit="return confirm('確定要刪除此服務嗎？');">
            @csrf
            @method('DELETE')
    ...
    <div class="row">
        <div class="col-lg-8">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text"
                               ...
                               maxlength="200">
                        <div class="form-text">
                            當前網址：{{ url('services/' . $service->slug) }}
                        </div>
                        @error('slug')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        @include('admin.media.partials.icon-picker-field', [
                            'fieldName'  => 'icon',
                            'fieldId'    => 'icon',
                            'fieldValue' => old('icon', $service->icon),
                        ])
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('excerpt') is-invalid @enderror"
                                  ...
                                  name="excerpt"
                                  rows="3"
                                  maxlength="500">{{ old('excerpt', $service->excerpt) }}</textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('content') is-invalid @enderror"
                                  id="content"
                                  name="content"
                                  rows="15">{{ old('content', $service->content) }}</textarea>
                        @error('content')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <div id="features-fields">
                            @foreach(old('features_items', $service->features ?? []) as $item)
                    ...
                    @php
                        $faqQuestions = old('faq_questions', collect($service->faq ?? [])->pluck('q')->all());
                        $faqAnswers = old('faq_answers', collect($service->faq ?? [])->pluck('a')->all());
                    @endphp
                    <div class="mb-3">
                        ...
                        <div id="faq-fields">
                            ...
                            <div class="card mb-2 faq-item">
                                <div class="card-body py-2">
                                    ...
                                    <textarea name="faq_answers[]" class="form-control" rows="2" placeholder="答案">{{ $faqAnswers[$i] ?? '' }}</textarea>
                                </div>
            ...
            <div class="card mt-3">
                ...
                <div class="card-body">
                    @php
                        $existingItems = old('items', $service->items->map(fn($i) => ['name' => $i->name, 'type' => $i->type])->toArray());
                    @endphp
        ...
        <div class="col-lg-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="type" name="type">
                            ...
                            <option value="website" {{ old('type', $service->type) === 'website' ? 'selected' : '' }}>網站方案</option>
                            <option value="hosting" {{ old('type', $service->type) === 'hosting' ? 'selected' : '' }}>主機代管</option>
                            <option value="maintenance" {{ old('type', $service->type) === 'maintenance' ? 'selected' : '' }}>維護服務</option>
                            <option value="addon" {{ old('type', $service->type) === 'addon' ? 'selected' : '' }}>加值服務</option>
                            <option value="consulting" {{ old('type', $service->type) === 'consulting' ? 'selected' : '' }}>顧問服務</option>
                        </select>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" id="billing_cycle" name="billing_cycle">
                                ...
                                <option value="once" {{ old('billing_cycle', $service->billing_cycle) === 'once' ? 'selected' : '' }}>一次性</option>
                                <option value="yearly" {{ old('billing_cycle', $service->billing_cycle) === 'yearly' ? 'selected' : '' }}>年繳</option>
                                <option value="monthly" {{ old('billing_cycle', $service->billing_cycle) === 'monthly' ? 'selected' : '' }}>月繳</option>
                                <option value="hourly" {{ old('billing_cycle', $service->billing_cycle) === 'hourly' ? 'selected' : '' }}>按時計費</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="pricing_category_id" name="pricing_category_id">
                            ...
                            @foreach($pricingCategories as $cat)
                                <option value="{{ $cat->id }}" {{ old('pricing_category_id', $service->pricing_category_id) == $cat->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text"
                               ...
                               id="tech_tags"
                               name="tech_tags"
                               value="{{ old('tech_tags', is_array($service->tech_tags) ? implode(', ', $service->tech_tags) : $service->tech_tags) }}"
            ...
            <div class="card mt-3">
                ...
                <div class="collapse {{ $service->pages_min || $service->revisions || $service->warranty_months || $service->work_days_min ? 'show' : '' }}" id="specCollapse">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-6 mb-3">
                                <label class="form-label">最少頁面</label>
                                <input type="number" name="pages_min" class="form-control" value="{{ old('pages_min', $service->pages_min) }}" min="0">
                            </div>
                            <div class="col-6 mb-3">
                                <label class="form-label">最多頁面</label>
                                <input type="number" name="pages_max" class="form-control" value="{{ old('pages_max', $service->pages_max) }}" min="0">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-6 mb-3">
                                <label class="form-label">修改次數</label>
                                <input type="number" name="revisions" class="form-control" value="{{ old('revisions', $service->revisions) }}" min="0">
                                <div class="form-text">留空=不限</div>
                            </div>
                            <div class="col-6 mb-3">
                                <label class="form-label">保固月數</label>
                                <input type="number" name="warranty_months" class="form-control" value="{{ old('warranty_months', $service->warranty_months) }}" min="0">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-6 mb-3">
                                <label class="form-label">最少工期（天）</label>
                                <input type="number" name="work_days_min" class="form-control" value="{{ old('work_days_min', $service->work_days_min) }}" min="0">
                            </div>
                            <div class="col-6 mb-3">
                                <label class="form-label">最多工期（天）</label>
                                <input type="number" name="work_days_max" class="form-control" value="{{ old('work_days_max', $service->work_days_max) }}" min="0">
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">設計方式</label>
                            <input type="text" name="design_method" class="form-control" value="{{ old('design_method', $service->design_method) }}" placeholder="例：客製化設計">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="order" name="order"
                               value="{{ old('order', $service->order) }}" readonly>
...
<script>
    ...
    // Items（包含項目）動態新增/移除
    var itemIndex = {{ count(old('items', $service->items->toArray())) }};
