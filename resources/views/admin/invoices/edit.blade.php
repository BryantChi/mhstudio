...
<div class="row mb-4">
    <div class="col-md-6">
        <h2 class="mb-0">編輯發票</h2>
        <p class="text-muted">{{ $invoice->invoice_number }}</p>
    </div>
    <div class="col-md-6 text-md-end">
        <a href="{{ route('admin.invoices.show', $invoice) }}" class="btn btn-light">查看</a>
    </div>
</div>

<form method="POST" action="{{ route('admin.invoices.update', $invoice) }}" onsubmit="showLoading()">
    @csrf @method('PUT')
    <div class="row">
        <div class="col-lg-8">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('title') is-invalid @enderror"
                               id="title" name="title" value="{{ old('title', $invoice->title) }}" placeholder="例如：官網設計第一期款" required>
                    ...
                    <div class="row mt-4 justify-content-end">
                        <div class="col-md-5">
                            <table class="table table-sm">
                                <tr><td>小計</td><td class="text-end" id="subtotal">NT$ {{ number_format($invoice->subtotal) }}</td></tr>
                                <tr><td>折扣</td><td class="text-end"><input type="number" class="form-control form-control-sm text-end" name="discount" id="discount" value="{{ old('discount', $invoice->discount) }}" min="0" step="0.01" style="width:120px;display:inline-block;"></td></tr>
                                <tr><td>稅率 (%)</td><td class="text-end"><input type="number" class="form-control form-control-sm text-end" name="tax_rate" id="taxRate" value="{{ old('tax_rate', $invoice->tax_rate) }}" min="0" max="100" step="0.01" style="width:80px;display:inline-block;"></td></tr>
                                <tr><td>稅額</td><td class="text-end" id="taxAmount">NT$ {{ number_format($invoice->tax_amount) }}</td></tr>
                                <tr class="fw-bold"><td>總計</td><td class="text-end" id="totalAmount">NT$ {{ number_format($invoice->total) }}</td></tr>
                            </table>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="notes" name="notes" rows="3" placeholder="付款資訊（銀行帳號等）">{{ old('notes', $invoice->notes) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-lg-4">
            <div class="card">
                ...
                <div class="card-body">
                    @include('admin.partials.client-select', ['clients' => $clients, 'selected' => old('client_id', $invoice->client_id)])
                    <div class="mb-3">
                        <label class="form-label">關聯專案</label>
                        <select class="form-select" name="project_id">
                            <option value="">不關聯</option>
                            @foreach($projects as $project)
                                <option value="{{ $project->id }}" {{ old('project_id', $invoice->project_id) == $project->id ? 'selected' : '' }}>{{ $project->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" class="form-control" name="issued_date" value="{{ old('issued_date', $invoice->issued_date->format('Y-m-d')) }}" required>
                        </div>
                        <div class="col-md-6 mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="currency" name="currency">
                            <option value="TWD" {{ old('currency', $invoice->currency) == 'TWD' ? 'selected' : '' }}>TWD (新台幣)</option>
                            <option value="USD" {{ old('currency', $invoice->currency) == 'USD' ? 'selected' : '' }}>USD (美元)</option>
                            <option value="CNY" {{ old('currency', $invoice->currency) == 'CNY' ? 'selected' : '' }}>CNY (人民幣)</option>
                        </select>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    itemsBody.addEventListener('click', function(e) { if (e.target.classList.contains('remove-item')) { e.target.closest('tr').remove(); updateRemoveButtons(); updateRowNumbers(); calculateTotal(); } });
    function updateRowNumbers() {
        document.querySelectorAll('.item-row').forEach((row, index) => {
            row.querySelector('.item-number').textContent = index + 1;
        });
    }
    function updateRemoveButtons() { const rows = itemsBody.querySelectorAll('.item-row'); rows.forEach(r => { r.querySelector('.remove-item').disabled = rows.length <= 1; }); }
    function bindCalculation() { document.querySelectorAll('.item-qty, .item-price').forEach(i => { i.removeEventListener('input', calculateTotal); i.addEventListener('input', calculateTotal); }); }
    function calculateTotal() {
        let subtotal = 0;
        document.querySelectorAll('.item-row').forEach(row => { const q = parseFloat(row.querySelector('.item-qty').value)||0; const p = parseFloat(row.querySelector('.item-price').value)||0; const a = q*p; row.querySelector('.item-amount').textContent = 'NT$ '+a.toLocaleString(); subtotal += a; });
        const d = parseFloat(document.getElementById('discount').value)||0, tr = parseFloat(document.getElementById('taxRate').value)||0;
        const tax = Math.round((subtotal-d)*(tr/100));
        ...
        document.getElementById('totalAmount').textContent = 'NT$ '+(subtotal-d+tax).toLocaleString();
    }
    bindCalculation(); updateRemoveButtons();
